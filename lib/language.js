...
function handleVueCode(code, languageUtils, exportName) {
  ...
  const sfcDescriptor = (0, _lodash.default)(originSfcDescriptor); // sfcDescriptor.template.content = handleTemplateCode(sfcDescriptor.template.content, languageUtils, exportName)
...
class LanguageUtils {
  ...
  stringToIdentifier(name, text, params = []) {
    ...
    const param = (0, _map2.default)(params).call(params, item => {
