...
function _default(code, languageUtils, exportName) {
  ...
  function handleStartTag(match) {
    ...
    for (let i = 0; i < l; i++) {
      ...
      if (value && (0, _common.isChineseChar)(value)) {
        ...
        handleReplaceChineseChar(value, start + (0, _indexOf.default)(text).call(text, value), true);
      }
  ...
  function handleReplaceChineseChar(text, start, isAttr = false) {
    // 其他字符串格式
    const zhMatch = text.match(chinese);

    while (zhMatch && zhMatch.length) {
      const char = zhMatch.shift();
      const charStart = start + (0, _indexOf.default)(text).call(text, char);
      const charEnd = charStart + char.length;
      const value = languageUtils.stringToIdentifier(exportName, char.replace(quote, '$1$2$3'));
      codeReplace(charStart, charEnd, isAttr ? value : `{{ ${value} }}`);
    }
  }
