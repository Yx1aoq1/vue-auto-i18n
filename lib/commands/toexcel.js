...
function initLanguagePaths(cfg) {
  ...
  if ((0, _common.isObject)(cfg.languagePath)) {
    ...
    return (0, _reduce.default)(_context2 = (0, _keys.default)(cfg.languagePath)).call(_context2, (target, lang) => {
...
function toexcel(program) {
  program.command('toexcel [jspath]').option('-n, --name <exportName>', '输出文件名称').option('-d, --dir <exportDir>', '输出文件位置').description('将i18n文件转成excel').action((jspath = '', {
    ...
    if (extname === 'js') {
      ...
      // 文件夹处理
      (0, _forEach.default)(_context3 = (0, _filter.default)(_context4 = _fs.default.readdirSync(filepath)).call(_context4, filename => filename !== 'index.js' && (0, _indexOf.default)(filename).call(filename, '.js') > -1)).call(_context3, filename => {
    ...
    if (buildDatas.length) {
      ...
      const exportFilePath = _path.default.join(exportDir, `${exportName}.xlsx`); // 确保目录存在
