...
function readESModuleFile(filePath) {
  ...
  const tempPath = _path.default.join(process.cwd(), './temp.js'); // 由于运行时不允许es6语法，只能替换一下再重新读取
...
function toexcel(program) {
  program.command('toexcel <jspath> [filename] [path]').description('将i18n文件转成excel').action((jspath, exportName, exportPath) => {
    ...
    _fs.default.access(fullPath, _fs.default.constants.F_OK, err => {
      if (err) {
        ...
        if (buildDatas.length) {
          ...
          const exportFilePath = _path.default.join(exportPath || '', `${exportName || 'translate'}.xlsx`); // 确保目录存在
